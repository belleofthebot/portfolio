...
(function (global) {
  ...
  function mossFigure(pct) {
    if (typeof pct !== 'number') {
      throw new Error('Moss is reserved for recycled figures, got ' + pct);
    }
  ...
  function shell(n, total, kicker, body, code, opts) {
    ...
    var srcbit = opts.src ? '<span class="fsrc">' + esc(opts.src) + '</span>' : '<span></span>';
    ...
    return '<section class="slide' + (opts.dark ? ' dark' : '') + '">' + head +
      ...
      '<div class="foot"><div class="foot-r1">' + srcbit +
  ...
  function buildSlides(fam, data) {
    ...
    S.push(shell(3, total, 'the product',
      ...
      '</div></div>', code, { src: 'description, tags and diagram · product record' }));
    ...
    S.push(shell(4, total, 'the range',
      ...
      '&times; depth order. The same on every document CambiumPak issues.</p>' +
      ...
      code, { src: 'every figure on this slide · spec sheet' }));
    ...
    if (skus.length >= 2 && distinct.length >= 2 &&
      ...
      src5 = 'board grades, burst and load · spec sheet';
    } else {
      body5 = '<h2>The specification, in full.</h2>' +
        '<p class="lede">Every tested figure, as it appears on the spec sheet. ' +
        'Provisional, and not independently certified.</p><div class="specgrid">' +
        fam.specification.concat(fam.measurements).map(function (d) {
          return '<div class="specrow"><span>' + esc(d.label) + '</span><b>' +
            esc(d.value) + '</b></div>';
        }).join('') + '</div>';
      src5 = 'specification and measurements · spec sheet';
    }
    S.push(shell(5, total, 'construction', body5, code, { src: src5 }));
    ...
    S.push(shell(6, total, 'what it is certified for',
      ...
      code, { src: 'what each listing certifies · features and benefits sheet' }));
    ...
    S.push(shell(7, total, 'recycled content',
      '<h2>Two facts, not one fact twice.</h2><div class="recycled"><div class="rec-l">' +
      '<p class="reclab">the figure, by weight</p><div class="recfigs">' + figs + '</div>' +
      '<p class="prov">Provisional, not independently certified.</p>' +
      '<p class="src">features and benefits sheet</p></div><div class="rec-r">' +
      ...
      'and neither stands in for the other.</p><p class="src">comparison sheet</p></div></div>',
      code, {}));

    /* 8 — at the pack bench */
    var fourth = docs.assembly.title, cells, lede8, gap8 = null;
    if (fam.steps.length) {
      ...
      lede8 = 'The first ' + fam.steps.length + ' of ' + fam.stepCount +
        '. Timings, handling cautions and what to do when it will not close are on the ' +
        fourth.toLowerCase() + '.';
    } else {
      cells = [1, 2, 3, 4].map(function (i) {
        return '<div class="step"><span class="sn">' + i +
          '</span><p class="nostep">step drawing not yet made</p></div>';
      }).join('');
      lede8 = 'The sequence is on the ' + fourth.toLowerCase() + '.';
      gap8 = 'step drawings for this family are not built yet';
    }
    S.push(shell(8, total, 'at the pack bench',
      '<h2>' + esc(fourth.split(' ')[0]) + ' in ' +
      (fam.stepCount || 'a few') + ' steps.</h2>' +
      '<p class="lede">' + esc(lede8) + '</p><div class="steps">' + cells + '</div>',
      code, { src: 'make-up sequence and drawings · ' + fourth.toLowerCase(), gap: gap8 }));

    /* 9 — the close */
    var order = [docs.spec, docs.comparison, docs.features, docs.assembly];
    var doclist = order.map(function (d) {
      return '<li><b>' + esc(d.title) + '</b><span>' + esc(d.question) + '</span><em>' +
        d.pages + ' pp</em></li>';
    }).join('');
    var pages = order.reduce(function (a, d) { return a + d.pages; }, 0);
    S.push(shell(9, total, null,
      '<div class="close"><div class="close-l">' +
      '<p class="cov-kick">Everything behind this deck</p>' +
      '<h2 class="closeh">One page. All four sheets.</h2>' +
      '<ul class="doclist">' + doclist + '</ul>' +
      '<p class="closefoot">' + pages + ' pages across four documents, plus the unified ' +
      'product record. That record is the one place every fact on these slides can be ' +
      'checked against its source.</p></div><div class="close-r">' +
      '<div class="qrbox">' + (fam.qr || '') + '</div>' +
      '<p class="qrurl">' + esc(fam.url.replace('https://', '')) + '</p>' +
